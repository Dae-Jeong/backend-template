...
describe('bootstrap and DI', () => {
  it('overrides clock per app and shares HTTP setup', async () => {
    const module = await Test.createTestingModule({ imports: [AppModule.register(readSettings({}))] })
      .overrideProvider(CLOCK).useValue(() => new Date('2026-01-01T00:00:00Z')).compile();
    const app = module.createNestApplication({ logger: false });
    configureApp(app);
    await app.init();
    try {
      const response = await request(app.getHttpServer()).get('/v1/greetings?name=Marin').expect(200);
      expect(response.body.data).toEqual({ message: 'Hello, Marin!', generated_at: '2026-01-01T00:00:00.000Z' });
    ...
    try { expect((await request(other.getHttpServer()).get('/v1/greetings?name=Marin')).body.data.generated_at).not.toBe('2026-01-01T00:00:00.000Z'); }
  ...
  it('closes readiness when listen fails', async () => {
    ...
    try {
      await expect(listen(app, owner.getHttpServer().address().port, '127.0.0.1')).rejects.toThrow();
      expect(app.get(Readiness).ready).toBe(false);
