...
Status: Task 1·2·3 완료 · 로그·오류 응답·계측 후속 · 2026-09-07
...
| 4 | HTTP metrics · 다음 작업 | 언어·환경에 맞는 계측·수집 라이브러리 선택, 요청 수·지연·결과, 제한된 라벨, 앱별 registry, 관측 실패·동시 요청 격리 | 4 |
...
먼저 [관측 설계](../observability.md)의 라이브러리 사용 원칙에 따라 Python/FastAPI 계측·수집 연동 후보를 확인합니다.
`/metrics`는 현재 로컬 후보이며 전체 언어·환경의 전달 방식을 고정하지 않습니다. 수집 대상이 미정이어도 라이브러리 검토와 로컬 지표 검증은 진행합니다.
