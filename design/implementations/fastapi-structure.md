...
아래는 향후 기능을 위한 설계 예시이며 현재 주문·재고·결제 구현이 있다는 뜻은 아닙니다.
한 방향 호출이면 필요한 도메인의 공개 계약을 사용하고 구현 함수는 조립 지점에서 주입합니다.
API router·Depends·ORM 모델을 다른 도메인의 업무 함수에서 가져다 쓰지 않습니다.
`contracts.py`로 타입을 옮기는 것만으로 순환이 해결되지는 않습니다. 계약끼리의 import와 실행 중 상호 호출도 확인합니다.

여러 도메인의 순서·실패 처리가 하나의 업무를 이루면 해당 업무의 `usecase.py`가 조합을 소유합니다.
예를 들어 주문 확정이 주문·재고·결제를 조합한다면 `checkout/usecase.py`의 함수가 Facade 역할을 할 수 있습니다.
기존 도메인이 그 업무를 명확히 소유하면 해당 도메인에 두며, 별도 `facades/` 폴더나 클래스는 필수가 아닙니다.

```mermaid
flowchart TD
    W["checkout/usecase.py · 주문 확정 조합 예시"] --> O["orders · 공개 업무 계약"]
    W --> I["inventory · 공개 업무 계약"]
    W --> P["payments · 공개 업무 계약"]
    BOOT["bootstrap · 구체 함수 연결"] -. "명시적 인자로 주입" .-> W
```

실선은 업무 호출 방향입니다. 참여 도메인은 조합 함수를 역으로 호출하지 않습니다.
A가 B를 호출하고 B가 다시 A의 결과를 요구한다면 조합 함수가 필요한 값을 구해 각각 전달하거나,
실제로 하나인 업무 경계를 다시 정합니다. 지연 import로 순환 업무 의존성을 숨기지 않습니다.
트랜잭션 소유권·재시도는 조합 단계에서 명시합니다. 동일 DB에서 원자성이 필요하면 같은 트랜잭션을 전달하고,
