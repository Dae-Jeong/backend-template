...
트랜잭션 경계는 HTTP 요청·WebSocket 연결·GraphQL 응답이 아니라 하나의 업무 단위입니다.
입력 경계는 업무를 호출하고 결과를 전달하며, 업무 실패의 rollback을 HTTP 상태나 응답 본문의 오류 판정에 맡기지 않습니다.
여러 작업을 조합하는 바깥쪽 업무가 원자적 범위를 소유하고 내부 작업은 그 범위에 참여합니다.
